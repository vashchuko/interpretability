...
class PathPatchingABC:
    ...
    def forward_run_c(self, 
        ...
        print('Run C')
        for token_position in tqdm(range(self.target_token_position + 1)):
                ...
                for head in range(self.model.cfg.n_heads):
                    ...
                    model_fwd_hooks.append(
                        ...
                        partial(head_patching_hook,
                                patching_values=run_b_cache[str(token_position)],
                                head=head,
                                token_position=token_position)
                        ))
                    ...
                    cache_dict_for_target_head[f'{layer}_{head}_{token_position}'][f'blocks.{self.target_layer}.attn.hook_{self.other_heads_activation_value_name}'] \
                        = cache_dict[f'blocks.{self.target_layer}.attn.hook_{self.other_heads_activation_value_name}'][-1, token_position, :, :].detach().clone()
    ...
    def forward_run_d(self, 
        ...
        for token_position in tqdm(range(self.target_token_position + 1)):
            ...
            patching_result[str(token_position)]['logit_diff'] = torch.zeros((self.model.cfg.n_layers, self.model.cfg.n_heads), dtype=torch.double, device=self.model.cfg.device)
            patching_result[str(token_position)]['expected_token_logit'] = torch.zeros((self.model.cfg.n_layers, self.model.cfg.n_heads), dtype=torch.double, device=self.model.cfg.device)
            patching_result[str(token_position)]['base_token_logit'] = torch.zeros((self.model.cfg.n_layers, self.model.cfg.n_heads), dtype=torch.double, device=self.model.cfg.device)
            ...
            for layer in range(self.get_layers_number(token_position)):
                for head in range(self.model.cfg.n_heads):
                    ...
                    patching_result[str(token_position)]['logit_diff'][layer, head] = (1.0 - patched_logit_diff/run_a_logit_diff)
                    if (run_a_expected_token_logit is not None and run_a_base_token_logit is not None):
                        ...
                        patching_result[str(token_position)]['expected_token_logit'][layer, head] = (1.0 - patched_expected_token_logit/run_a_expected_token_logit)
                        patching_result[str(token_position)]['base_token_logit'][layer, head] = (1.0 - patched_base_token_logit/run_a_base_token_logit)
...
class PathPatchingWithExternalCache(PathPatchingABC):
    ...
    def run(self,
        ...
        target_head_cache = \
            self.forward_run_c(prompt=run_a_prompt, run_a_cache=run_a_cache, run_b_cache=external_cache)
        ...
        run_d_patching_result = self.forward_run_d(prompt=run_a_prompt, expected_token=expected_token, base_token=base_token, \
                        ...
                        run_a_expected_token_logit=run_a_expected_token_logit, \
                        run_a_base_token_logit=run_a_base_token_logit)
        
        return run_d_patching_result
